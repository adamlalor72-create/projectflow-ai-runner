...
// On local: uses a profile dir so Chromium remembers cert selections.
// On CF: ephemeral filesystem — use launchPersistentContext with a tmp dir,
//        or fall back to regular launch if the tmp approach fails.
let _context = null;
const USER_DATA_DIR = config.isCloudFoundry
  ? "/tmp/dealflow-runner-profile"
  : path.join(os.homedir(), ".dealflow-runner-profile");

export async function launchBrowser() {
  if (_context) return _context;
  console.log("[Browser] Launching Chromium (headless:", config.browser.headless, ", CF:", config.isCloudFoundry, ")...");
  ...
  if (config.isCloudFoundry) {
    // On CF: regular launch (no persistent profile — ephemeral filesystem)
    console.log("[Browser] CF mode: using regular browser launch (no persistent profile)");
  ...
  // Local: persistent context (remembers cert selections etc.)
  const launchOptions = {
  ...
  _context = await chromium.launchPersistentContext(USER_DATA_DIR, launchOptions);
  _context.setDefaultTimeout(config.browser.timeout);
  return _context;
}
...
/**
 * Takes a screenshot and returns { path, buffer }.
 * On CF: saves to /tmp, logs base64, returns the /tmp path.
 * On local: saves to screenshotDir, returns the path.
 * Always returns a real file path so callers can readFile() it.
 */
export async function screenshot(page, name) {
  try {
    ...
    if (config.isCloudFoundry) {
      // CF: save to /tmp (ephemeral but readable within same process)
      const filepath = path.join("/tmp/screenshots", filename);
      await mkdir("/tmp/screenshots", { recursive: true });
      await page.screenshot({ path: filepath, fullPage: true });
      // Also log as base64 so it's retrievable from kubectl logs
      const { readFile: rf } = await import("fs/promises");
      const b64 = (await rf(filepath)).toString("base64");
      console.log(`[SCREENSHOT:${name}_${ts}] data:image/png;base64,${b64}`);
      return filepath;
    } else {
      // Local: save to screenshots directory
      await mkdir(config.browser.screenshotDir, { recursive: true });
      const filepath = path.join(config.browser.screenshotDir, filename);
      await page.screenshot({ path: filepath, fullPage: true });
